...
n + 1 = S n.
Proof.
  apply nat_ind. reflexivity. intros n' IHn'. simpl. f_equal.
  apply IHn'.
Qed.
...
(*          (P: t_tree X -> Prop)  *)
(*          (ev_leaf: P t_leaf) *)
(*          (ev_branch: forall v:X, forall l: t_tree X, P l -> forall r: t_tree X, P r -> P (t_branch (l, v, r))) *)
(*          (t: t_tree X) *)
         
Definition better_t_tree_ind : better_t_tree_ind_type :=
  fun X => fun P => fun PLeaf => fun PBranch => fix f (t: t_tree X) :=
    match t with
    | t_leaf => PLeaf
