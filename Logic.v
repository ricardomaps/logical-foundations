...
Proof. split; [destruct n | destruct m]; try reflexivity; try discriminate;
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H contra p. apply H in p. apply contra in p.
  destruct p.
Qed.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P [HP HNP]. apply HNP in HP. destruct HP.
Qed.
(** [] *)
...
Proof.
  intros P Q H. split.
  - intros p. apply or_intro_l with (B := Q) in p. apply H in p. destruct p.
  - intros q. apply or_intro_l with (B := P) in q. apply or_commut in q. apply H in q. destruct q.
Qed.    
...
Proof.
  ...
  - intros p. apply H1 in p. apply H2 in p. apply p.
  - intros r. apply H2 in r. apply H1 in r. apply r.
Qed.
...
Proof.
  intros X P Hx [x Ex]. apply Ex in Hx. destruct Hx.
Qed.
(** [] *)
...
Proof.
  ...
  - intros [[x1 Px] | [x2 Qx]].
    + exists x1. left. apply Px.
    + exists x2. right. apply Qx.
Qed.
...
Proof.
  ...
  - intros m H. destruct m as [| m'].
    ...
    + apply IHn' in H. simpl. destruct H as [x Ex].
      exists x. rewrite Ex. reflexivity.
Qed.
...
Proof.
  ...
  - intros m [x Ex]. destruct m as [|m'].
    + discriminate.
    + apply IHn'. exists x. simpl in Ex. apply S_injective in Ex. apply Ex.
Qed.
...
Proof.
  ...
  - split.
    + simpl. intros [H1 | H2].
      left. left. apply H1.
      apply IH in H2. destruct H2 as [H21 | H22].
      left. right. apply H21.
      right. apply H22.
    + simpl. intros [[H1 | H2] | H3].
      left. apply H1.
      apply or_intro_l with (B := In a l'0) in H2. apply IH in H2.
      right. apply H2.
      apply or_intror with (A := In a l') in H3. apply IH in H3.
      right. apply H3.
Qed.
(** [] *)
...
Theorem All_In :
  forall T (P : T -> Prop) (l : list T),
    ...
    All P l.
Proof.
  intros T P l. induction l as [|n l' IHl'].
  - simpl. split.
    + intros H. apply I.
    + intros H T' [].
  - simpl. split.
    ...
    + intros [H1 H2] x [H3 | H4]. rewrite <- H3. apply H1.
      apply IHl' in H4. apply H4. apply H2.
Qed.
...
Proof.
  ...
  - rewrite even_S. destruct (even n') eqn:E.
      + simpl. exists k. rewrite IH. reflexivity.
      + simpl. exists (S k). rewrite IH. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros b1 b2. split.
  - intros H. destruct b1. left. reflexivity. simpl in H. right. apply H.
  - intros [H1 | H2].
...
Proof.
  intros x y. split.
  - intros H1 H2. rewrite <- eqb_eq in H2. rewrite H2 in H1. discriminate H1.
  - intros H. destruct (x =? y) eqn:E.
    rewrite <- eqb_eq in H. apply H in E. destruct E.
    reflexivity. 
Qed.
(** [] *)
...
Proof.
  ...
  - generalize dependent l2. induction l1 as [|n1 l1' IH].
    + destruct l2. reflexivity. discriminate.
    + destruct l2 as [|n2 l2'].
      discriminate.
      intros H1. simpl in H1. rewrite andb_true_iff in H1. destruct H1 as [H1 H2]. apply H in H1. apply IH in H2. rewrite H1. rewrite H2. reflexivity.
  - generalize dependent l2. induction l1 as [|n1 l1' IH].
    + intros l2 H1. rewrite <- H1. reflexivity.
    + destruct l2 as [|n2 l2'] eqn:E.
      * intros H1. rewrite H1. reflexivity.
      * intros H1. simpl. injection H1 as H2 H3. rewrite andb_true_iff.
        split. apply H. apply H2. apply IH. apply H3.
Qed.
...
Proof.
  ...
  - intros. induction l as [| n l' IH].
    ...
    + simpl. simpl in H. apply andb_true_iff in H. destruct H as [H1 H2].
      apply IH in H2. split.
      * apply H1.
      * apply H2.
  - intros. induction l as [|n l' IH].
    + reflexivity.
    + simpl in H. destruct H as [H1 H2]. simpl. apply andb_true_iff. split.
      * apply H1.
      * apply IH. apply H2.
Qed.
...
Lemma rev_app_eq: forall (X: Type) (l l1 l2: list X),
    rev_append l (l1 ++ l2) = rev_append l l1 ++ l2.
Proof.
  induction l.
  - reflexivity.
  - intros. simpl. apply IHl with (l1 := x :: l1).
Qed.

Theorem tr_rev_correct : forall X, @tr_rev X = @rev X.
Proof.
  intros. apply functional_extensionality. 
  induction x as [|n x' IH].
  - reflexivity.
  - unfold tr_rev in *. simpl. rewrite <- IH. apply rev_app_eq with (l1 := []).
Qed.    
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof.
  unfold not. intros. apply de_morgan_not_or in H. destruct H as [H1 H2]. contradiction.
Qed.
...
Proof.
   intros EM X P H1 x. destruct (EM (P x)).
  - apply H.
  - destruct H1. exists x. apply H.
Qed.
...
Proof.
  ...
  intros contra. apply H0 in contra. destruct contra.
Qed.

Theorem dne_dmnan: double_negation_elimination -> de_morgan_not_and_not.
Proof.
  unfold double_negation_elimination. unfold de_morgan_not_and_not.
  intros dne P Q dmnan. apply dne. unfold not. intros. apply de_morgan_not_or in H. apply dmnan in H. destruct H.
Qed.

Theorem dmnan_ito: de_morgan_not_and_not -> implies_to_or.    
Proof.
  unfold de_morgan_not_and_not. unfold implies_to_or. intros.
  apply H. unfold not. intros. destruct H1 as [H1 H2].
  apply H1. intros. apply H0 in H3. apply H2 in H3. destruct H3.
Qed.

Theorem ito_em: implies_to_or -> excluded_middle.
Proof.
  unfold implies_to_or. unfold excluded_middle. intros.
  apply or_comm. apply H. intros. apply H0.
Qed.
