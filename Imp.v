...
Proof.
  ...
  - intros. generalize dependent bv. induction b;
    try (intros; simpl in H; rewrite <- H; constructor);
    try (induction bv; apply aeval_iff_aevalR);
...
Proof.
  ...
  - apply E_WhileTrue with (X !-> 1; Y !-> 2; Y !-> 0; X !-> 2).
    + reflexivity.
    + apply E_Seq with (Y !-> 2; Y !-> 0; X !-> 2); apply E_Asgn;
        reflexivity.
...
Proof.
  ...
  induction contra; try discriminate; inversion Heqloopdef.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | NW_Asgn v x : no_whilesR (CAsgn v x)
  | NW_Seq c1 c2 : no_whilesR c1 -> no_whilesR c2 -> no_whilesR (CSeq c1 c2)
  | NW_If b ct cf : no_whilesR ct -> no_whilesR cf -> no_whilesR (CIf b ct cf)
.
...
Proof.
  ...
  - intros. induction c;
    try constructor;
    inversion H; apply andb_prop in H1; destruct H1 as [H1 H2];
    try apply (IHc1 H1); try apply (IHc2 H2).
  - intros. induction H;
    try reflexivity;
...
Fixpoint s_execute (st : state) (stack : list nat) (prog : list sinstr): list nat :=
  ...
  | SPlus :: rest => match stack with
             | n1 :: n2 :: stk => s_execute st ((n2 + n1) :: stk) rest
...
Proof.
  intros. generalize dependent stack. induction p1 as [|inst p1' IH].
  - reflexivity.
  - destruct inst; intros; simpl;
    repeat (try apply IH; destruct stack).
Qed.
...
Proof.
  ...
  intros; simpl;
  try (repeat (rewrite execute_app); rewrite IHe1; rewrite IHe2);
  reflexivity.
Qed.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_WhileTrueContinue : forall st st' st'' s b c,
      beval st b = true ->
      st  =[ c ]=> st' / SContinue ->
...
Theorem seq_stops_on_break : forall c1 c2 st st',
  ...
  st =[ c1 ; c2 ]=> st' / SBreak.
Proof.
  intros. apply E_SeqBreak. apply H.
Qed.
...
Proof.
  intros. remember <{ while b do c end}> as defwhile. induction H;
    try discriminate Heqdefwhile;
    injection Heqdefwhile; intros; subst.
  - congruence.
  - apply IHceval2.
    + reflexivity.
    + apply H0.
