...
Theorem t_update_neq : forall (A : Type) (m : total_map A) x1 x2 v,
  ...
  (x1 !-> v ; m) x2 = m x2.
Proof.
  intros A m x1 x2 v H. unfold t_update. apply eqb_neq in H.
...
Proof.
  ...
  intros y. unfold t_update. destruct (x =? y)%string.
  - reflexivity.
  - reflexivity.
Qed.
...
Theorem t_update_same : forall (A : Type) (m : total_map A) x,
  (x !-> m x ; m) = m.
Proof.
  intros A m x. apply functional_extensionality. intros y.
  unfold t_update. destruct (eqb_spec x y).
  - subst. reflexivity.
  - reflexivity.
Qed.
...
Proof.
  ...
  intros y. unfold t_update. destruct (eqb_spec x1 y).
  - rewrite e in H. apply eqb_neq in H. rewrite H. reflexivity.
  - destruct (x2 =? y)%string.
    + reflexivity.
    + reflexivity.
Qed.
