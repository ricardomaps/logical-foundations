...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | [] => []
  | h :: tl => if odd h then h :: oddmembers tl else oddmembers tl
  end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | [] => 0
  | h :: tl => if h =? v then S (count v tl) else count v tl
  end.
...
Fixpoint member (v : nat) (s : bag) : bool :=
  match s with
  | [] => false
  | h :: tl => if h =? v then true else member v tl
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | [] => []
  | h :: tl => if h =? v then tl else h :: remove_one v tl
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | [] => []
  | h :: tl => if h =? v then remove_all v tl else h :: remove_all v tl
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | [] => true
  | h :: tl => if member h s2 then included tl (remove_one h s2) else false
  end.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | [], [] => true
  | h1 :: tl1, h2 :: tl2 => if h1 =? h2 then eqblist tl1 tl2 else false
  | _, _ => false
  end.
...
Proof.
  intros f Hn n1 n2 H. rewrite Hn. rewrite <- H. apply Hn.
Qed.
...
Proof.
  intros x. destruct x. simpl. apply eqb_refl.
Qed.
