...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros n m o p eq1 eq2. rewrite eq2. apply eq1.
Qed.
...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y z l j eq1 eq2. rewrite eq2 in eq1.
...
Proof.
  ...
  - intros m eq. destruct m as [|m'].
    + discriminate.
    + f_equal. apply IHn' in eq. apply eq.
Qed.

Theorem eqb_true' : forall n m,
  n =? m = true -> n = m.
Proof.
  intros n m eq. destruct n as [|n'].
  - destruct m as [|m'].
    + reflexivity.
    + discriminate.
  - destruct m as [|m'].
...
Proof.
  ...
  - intros m eq. simpl in eq.
    destruct m as [|m'].
    + discriminate.
    + simpl in eq. repeat rewrite <- plus_n_Sm in eq. f_equal. injection eq. apply IHn'.
Qed.
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
  ...
  nth_error l n = None.
Proof.
  intros n X l. generalize dependent n. induction l as [|h tl IHtl].
  - intros n eq. reflexivity.
  - intros n eq. destruct n as [|n'].
    + simpl. discriminate.
    + simpl. simpl in eq. injection eq. apply IHtl.
Qed.
...
Proof.
  ...
  - intros l1 l2 H. symmetry in H. injection H as H1 H2.
    rewrite H1, H2. reflexivity.
  - destruct n as [n1 n2]. simpl. destruct (split l'). intros l1 l2 H.
    symmetry in H. injection H as H1 H2. rewrite H1, H2. simpl.
...
Lemma conv_eqb_true : forall n m, n = m -> (n =? m) = true.
Proof.
  intros n m eq. rewrite eq. apply eqb_refl.
Qed.
...
n =? p = true.
Proof.
  intros n m p eq1 eq2. apply eqb_true in eq1, eq2.
  apply conv_eqb_true. transitivity m. apply eq1. apply eq2.
Qed.
...
Proof.
  intros X Y. induction l1 as [|n1 l1' IHl1'].
  - intros l2 H. destruct l2 as [|n2 l2'].
    + simpl.
...
test x = true.
Proof.
  intros X test x l. induction l as [|n l' IHl'].
  - simpl. intros lf H. discriminate H.
  - simpl. destruct (test n) eqn:E.
    + intros lf H. injection H as Eq1 Eq2. rewrite <- Eq1. apply E.
    + apply IHl'.
Qed.
...
Fixpoint forallb {X : Type} (test : X -> bool) (l : list X) : bool :=
  match l with
  | [] => true
  | h :: tl => if test h then forallb test tl else false
  end.
...
Fixpoint existsb {X : Type} (test : X -> bool) (l : list X) : bool :=
  match l with
  | [] => false
  | h :: tl => if test h then true else existsb test tl
  end.
...
Proof.
  intros X test l. induction l as [|n l' IHl'].
  - reflexivity.
  - simpl. destruct (test n) eqn:E1.
    + unfold existsb'. simpl. rewrite E1. simpl. reflexivity.
    + unfold existsb'. simpl. rewrite E1. simpl. apply IHl'.
Qed.
